...
public class DataConverter {
    ...
    private Object preProcessMapValue(final Object value, final Schema schema, final Schema newSchema) {
        ...
        for (final Map.Entry<?, ?> entry : map.entrySet()) {
            ...
            final Schema mapKeySchema = newValueSchema.field(ElasticsearchSinkConnectorConstants.MAP_KEY).schema();
            final Schema mapValueSchema = newValueSchema.field(ElasticsearchSinkConnectorConstants.MAP_VALUE).schema();
            mapStruct.put(
                ElasticsearchSinkConnectorConstants.MAP_KEY,
                preProcessValue(entry.getKey(), keySchema, mapKeySchema));
            mapStruct.put(
                ElasticsearchSinkConnectorConstants.MAP_VALUE,
